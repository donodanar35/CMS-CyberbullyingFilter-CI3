...
class MyModel extends CI_Model {
	...
	public function laporan_rating($id="", $tglawal="", $tglakhir=""){
		$this->db->select('AVG(RATING) AS RATING, TANGGAL_RATING AS TANGGAL');
		$this->db->from('rating');
		$this->db->where('AKTIF','Y');
		$this->db->where('ID_POSTING',$id);
		if($tglawal <> "" && $tglakhir <> ""){
			$this->db->where('TANGGAL_RATING >=',$tglawal);
			$this->db->where('TANGGAL_RATING <=',$tglakhir);
		}
	...
	public function laporan_artikel($id="", $tglawal="", $tglakhir=""){
		$this->db->select('COUNT(VIEW) AS VIEW, TANGGAL_VIEW AS TANGGAL');
		$this->db->from('view');
		$this->db->where('AKTIF','Y');
		$this->db->where('ID_POSTING',$id);
		if($tglawal <> "" && $tglakhir <> ""){
			$this->db->where('TANGGAL_VIEW >=',$tglawal);
			$this->db->where('TANGGAL_VIEW <=',$tglakhir);
		}
	...
	public function laporan_tayangan($id="", $tglawal="", $tglakhir=""){
		$this->db->select('COUNT(VIEW) AS VIEW, TANGGAL_VIEW AS TANGGAL');
		$this->db->from('view');
		$this->db->where('AKTIF','Y');
		$this->db->where('ID',$id);
	...
	public function getvalidasi_datasetkomentar($limit,$offset){
		$this->db->select('*'); 
		$this->db->from('dataset_komentar');
		$this->db->join('dataset_artikel', 'dataset_artikel.ID_ARTIKEL = dataset_komentar.ID_ARTIKEL' , 'left' );
	...
	public function totalvalidasi_datasetkomentar(){
		$this->db->from('dataset_komentar');
		$this->db->where('AKTIF','Y');
		$query = $this->db->get();
        return $query->num_rows();
	...
	public function getTopArtikel($id=0, $tglawal="", $tglakhir=""){
		if($tglawal <> "" && $tglakhir <> ""){
			$query = $this->db->query("
			SELECT posting.TANGGAL AS TANGGAL, pengguna.FIRST_NAME AS FIRST_NAME, pengguna.LAST_NAME AS LAST_NAME, posting.JUDUL_URL, posting.JUDUL AS JUDUL, SUM(view.VIEW) AS TOTAL, kategori.NAMA_KATEGORI AS NAMA_KATEGORI FROM posting, view, pengguna, kategori WHERE posting.ID_POSTING = view.ID_POSTING AND posting.ID_KATEGORI=kategori.ID_KATEGORI AND posting.ID=pengguna.ID AND posting.AKTIF='Y' AND pengguna.ID='". $id ."' AND view.TANGGAL_VIEW >='". $tglawal ."' AND view.TANGGAL_VIEW <='". $tglakhir ."' group by posting.ID_POSTING order by TOTAL DESC limit 5");
			return $query->result();
		}else{
			$query = $this->db->query("
			SELECT posting.TANGGAL AS TANGGAL, pengguna.FIRST_NAME AS FIRST_NAME, pengguna.LAST_NAME AS LAST_NAME, posting.JUDUL_URL, posting.JUDUL AS JUDUL, SUM(view.VIEW) AS TOTAL, kategori.NAMA_KATEGORI AS NAMA_KATEGORI FROM posting, view, pengguna, kategori WHERE posting.ID_POSTING = view.ID_POSTING AND posting.ID_KATEGORI=kategori.ID_KATEGORI AND posting.ID=pengguna.ID AND posting.AKTIF='Y' AND pengguna.ID='". $id ."' group by posting.ID_POSTING order by TOTAL DESC limit 5");
			return $query->result();
		}
	}

	public function getTopRating($id=0, $tglawal="", $tglakhir=""){
		if($tglawal <> "" && $tglakhir <> ""){
			$query = $this->db->query("
			SELECT AVG(rating.RATING) AS RATING, posting.TANGGAL AS TANGGAL, pengguna.FIRST_NAME AS FIRST_NAME, pengguna.LAST_NAME AS LAST_NAME, posting.JUDUL_URL, posting.JUDUL AS JUDUL, SUM(view.VIEW) AS TOTAL, kategori.NAMA_KATEGORI AS NAMA_KATEGORI FROM posting, view, pengguna, kategori, rating WHERE posting.AKTIF='Y' AND posting.ID_POSTING = rating.ID_POSTING AND posting.ID_POSTING = view.ID_POSTING AND posting.ID_KATEGORI=kategori.ID_KATEGORI AND posting.ID=pengguna.ID AND pengguna.ID='". $id ."' AND rating.TANGGAL_RATING >='". $tglawal ."' AND rating.TANGGAL_RATING <='". $tglakhir ."' group by posting.ID_POSTING order by RATING DESC limit 5");
			return $query->result();
		}else{
			$query = $this->db->query("
			SELECT AVG(rating.RATING) AS RATING, posting.TANGGAL AS TANGGAL, pengguna.FIRST_NAME AS FIRST_NAME, pengguna.LAST_NAME AS LAST_NAME, posting.JUDUL_URL, posting.JUDUL AS JUDUL, SUM(view.VIEW) AS TOTAL, kategori.NAMA_KATEGORI AS NAMA_KATEGORI FROM posting, view, pengguna, kategori, rating WHERE posting.AKTIF='Y' AND posting.ID_POSTING = rating.ID_POSTING AND posting.ID_POSTING = view.ID_POSTING AND posting.ID_KATEGORI=kategori.ID_KATEGORI AND posting.ID=pengguna.ID AND pengguna.ID='". $id ."' group by posting.ID_POSTING order by RATING DESC limit 5");
			return $query->result();
		}
	}
    ...
    public function pengguna_nonaktifkan($data){
        if($data['active']==1){
            $dataUbah = array(
			    'ACTIVE' => '0'
		    );    
		...
		$this->db->where('ID_USERS', $data['iduser']);
		$this->db->update('pengguna',$dataUbah);
		return $this->db->affected_rows();
	}
	...
	public function artikel_buat($data){
        ...
        $tinjau = "Y";
        $tinjau_artikel = $this->getidentitas();
		if($tinjau_artikel->TINJAU_ARTIKEL == "Y"){
		    $tinjau = "T";
		}else{
		    $tinjau = "Y";
		}
		...
		if(! empty($data['pathfoto']) && (! is_null($data['pathfoto']))){
			$dataBuat = array(
				...
				'JUDUL' => $this->security->xss_clean($data['judul']),
				"JUDUL_URL" => $judulUrl,
				'ISI_ARTIKEL' => $this->security->xss_clean($data['isiartikel']),
				"PHOTO" => $data['pathfoto'],
				"DESKRIPSI_ARTIKEL" => $this->security->xss_clean($data['deskripsiartikel']),
				"KATA_KUNCI" => $this->security->xss_clean($data['katakunci']),
	...
	public function artikel_byKategori($kategori,$limit,$offset){
		...
		$this->db->join ('kategori', 'kategori.ID_KATEGORI = posting.id_kategori' , 'left');
		$this->db->join('pengguna', 'posting.ID = pengguna.ID' , 'left' );
		$this->db->where('pengguna.ACTIVE','1');
		$this->db->like('posting.AKTIF','Y');
		$this->db->like('kategori.NAMA_KATEGORI',$kategori);
		$this->db->limit($limit, $offset);
		$this->db->order_by('ID_POSTING','DESC');
	...
	public function artikel($id){
		...
		$this->db->join('pengguna', 'posting.ID = pengguna.ID' , 'left' );
		$this->db->where('pengguna.ACTIVE','1');
		$this->db->where("(posting.AKTIF='Y' or posting.AKTIF='T')");
		$this->db->where('posting.ID',$id);
		$this->db->order_by('ID_POSTING','DESC');
        $query = $this->db->get();
        return $query;
	...
	public function allartikel(){
		//$this->db->select('*');
		$this->db->select('posting.ID_POSTING AS ID_POSTING, kategori.ID_KATEGORI AS ID_KATEGORI, posting.JUDUL AS JUDUL, posting.JUDUL_URL AS JUDUL_URL, posting.KATA_KUNCI AS KATA_KUNCI, posting.ISI_ARTIKEL AS ISI_ARTIKEL, posting.PHOTO AS PHOTO, posting.TANGGAL AS TANGGAL, posting.DESKRIPSI_ARTIKEL AS DESKRIPSI_ARTIKEL, posting.VIEW AS VIEW, posting.RATING AS RATING, posting.JUMLAH_RATING AS JUMLAH_RATING, posting.TOTAL_KOMENTAR AS TOTAL_KOMENTAR, posting.AKTIF AS AKTIF, kategori.NAMA_KATEGORI AS NAMA_KATEGORI, pengguna.ID_USERS AS ID_USER');
		$this->db->from('posting');
		$this->db->join('kategori', 'kategori.ID_KATEGORI = posting.id_kategori' , 'left' );
		$this->db->join('pengguna', 'posting.ID = pengguna.ID' , 'left' );
		$this->db->where('pengguna.ACTIVE','1');
		//$this->db->where('posting.AKTIF','T');
		$this->db->where("(posting.AKTIF='Y' or posting.AKTIF='T')");
		$this->db->order_by('ID_POSTING','DESC');
        $query = $this->db->get();
        return $query;
	...
	public function segmentasix($segmentasi){
		$tpx = 0;
		$fpx = 0;
		$fnx = 0;
		$tnx = 0;
	...
	public function tambah_riwayatkomentar($id,$idkomentar,$sentimen,$jenisbully,$keterangan){
		$timezone = new DateTimeZone('Asia/Jakarta');
		$tgl = new DateTime();
		$tgl->setTimeZone($timezone);
		...
		$dataBuat = array(
			"ID_PENGGUNA" => $id,
			"ID_DATASETKOMENTAR" => $idkomentar,
			"WAKTU" => $tgl->format('Y-m-d H:i:s'),
			"SENTIMEN" => $sentimen,
			"JENIS_CYBERBULLYING" => $jenisbully,
			"KETERANGAN" =>$keterangan,
			"AKTIF" => 'Y'
		);
		$this->db->insert('riwayat_validasi', $dataBuat);
	}

	public function validasi_komentar($idkomentar, $sentimen, $jenisbully, $keterangan){
		$dataUbah = array();
		if($sentimen=="cyberbullying"){
			$dataUbah = array(
			...
			$dataUbah = array(
				'SENTIMEN' => "non-cyberbullying",
				"JENIS_CYBERBULLYING" => "bukan perundungan"
			);
		}
		
        $this->db->where('ID_DATASETKOMENTAR', $idkomentar);
        $this->db->update('dataset_komentar', $dataUbah);
        return $this->db->affected_rows();
	}

	public function simpan_validasi_komentar($idkomentar, $isikomentar, $sentimen, $jenisbully){
		$dataUbah = array();
		if($sentimen=="cyberbullying"){
			$dataUbah = array(
	...
	public function get_validasibyid($id){
		$this->db->from('riwayat_validasi');
		$this->db->join('pengguna', 'pengguna.ID = riwayat_validasi.ID_PENGGUNA' , 'left' );
		$this->db->where('riwayat_validasi.AKTIF','Y');
		$this->db->where('ID_DATASETKOMENTAR',$id);
		$query = $this->db->get();
        return $query->result();
	}
	...
	public function posting_tinjau($data){
		if(! empty($data['pathfoto']) && (! is_null($data['pathfoto']))){
			$dataUbah = array(
				'ID' => $data['idpengguna'],
				'ID_KATEGORI' => $data['kategori'],
	...
	public function get_komentarArtikelByUser($id){
		$this->db->select('pengguna.ID AS ID, pengguna.FIRST_NAME AS FIRST_NAME, pengguna.LAST_NAME AS LAST_NAME, komentar.ID_KOMENTAR AS ID_KOMENTAR, komentar.KOMENTAR AS KOMENTAR, komentar.TANGGAL_KOMENTAR AS TANGGAL_KOMENTAR, komentar.SENTIMEN AS SENTIMEN, komentar.LAPORKAN AS LAPORKAN, komentar.PELANGGARAN AS PELANGGARAN, komentar.KETERANGAN AS KETERANGAN, komentar.TANGGAL_PELAPORAN AS TANGGAL_PELAPORAN, komentar.LAPORKAN, komentar.AKTIF, posting.ID_POSTING AS ID_POSTING');
		$this->db->from('komentar');
		$this->db->join('posting', 'posting.ID_POSTING = komentar.ID_POSTING' , 'left');
		$this->db->join('pengguna', 'komentar.ID = pengguna.ID' , 'left');
		$this->db->where('komentar.AKTIF','Y');
		...
		//$this->db->select('pengguna.ID AS ID, pengguna.FIRST_NAME AS FIRST_NAME, pengguna.LAST_NAME AS LAST_NAME, komentar.ID_KOMENTAR AS ID_KOMENTAR, komentar.KOMENTAR AS KOMENTAR, komentar.TANGGAL_KOMENTAR AS TANGGAL_KOMENTAR, komentar.SENTIMEN AS SENTIMEN, komentar.LAPORKAN AS LAPORKAN, komentar.PELANGGARAN AS PELANGGARAN, komentar.KETERANGAN AS KETERANGAN, komentar.TANGGAL_PELAPORAN AS TANGGAL_PELAPORAN, komentar.LAPORKAN, komentar.AKTIF');
		/*$this->db->from('komentar');
		$this->db->join('pengguna', 'pengguna.ID = komentar.ID' , 'left');
		$this->db->join('posting', 'posting.ID = pengguna.ID' , 'left');
		$this->db->join('komentar', 'komentar.ID_POSTING = posting.ID_POSTING' , 'left');
		$this->db->where('komentar.AKTIF','Y');
		$this->db->where('posting.ID',$id);
		$this->db->order_by('komentar.TANGGAL_KOMENTAR', 'DESC');
		$query = $this->db->get();
		return $query;
		$query = $this->db->query("SELECT pengguna.ID AS ID, pengguna.FIRST_NAME AS FIRST_NAME, pengguna.LAST_NAME AS LAST_NAME, komentar.ID_KOMENTAR AS ID_KOMENTAR, komentar.KOMENTAR AS KOMENTAR, komentar.TANGGAL_KOMENTAR AS TANGGAL_KOMENTAR, komentar.SENTIMEN AS SENTIMEN, komentar.LAPORKAN AS LAPORKAN, komentar.PELANGGARAN AS PELANGGARAN, komentar.KETERANGAN AS KETERANGAN, komentar.TANGGAL_PELAPORAN AS TANGGAL_PELAPORAN, komentar.LAPORKAN, komentar.AKTIF FROM posting, pengguna, komentar WHERE posting.ID_POSTING = komentar.ID_POSTING AND pengguna.ID = posting.ID AND pengguna.ID = komentar.ID AND posting.AKTIF ='Y' GROUP BY posting.ID_POSTING ORDER BY posting.ID_POSTING");
		$query = $this->db->get();
		return $query;*/
	}
	...
	public function buat_penilai($nama, $email, $gender, $profesi){
		...
		$data = array(
			"KODE" => $kode,
			"NAMA_PENILAI" => $this->security->xss_clean($nama),
			"EMAIL_PENILAI" => $this->security->xss_clean($email),
			"GENDER" => $gender,
			"PROFESI" => $profesi,
			"AKTIF" => 'Y',
	...
	public function buat_kritiksaran($id_penilai, $jawaban){
		...
		$tgl->setTimeZone($timezone);
		$data = array(
			"KRITIK_SARAN" => $this->security->xss_clean($jawaban),
			"ID_PENILAI" => $id_penilai,
			"TANGGAL" => $tgl->format('Y-m-d'),
			"AKTIF" => 'Y'
		);
	...
	public function komentar($idpengguna=null, $idposting=null,$komentar=null,$sentimen=null){
		$timezone = new DateTimeZone('Asia/Jakarta');
		$tgl = new DateTime();
		$tgl->setTimeZone($timezone);
		$data = array(
			"ID" => $idpengguna,
			"ID_POSTING" => $idposting,
			"KOMENTAR" => $this->security->xss_clean($komentar),
			"AKTIF" => 'Y',
	...
	public function balasan_komentar($idpengguna=null, $idkomentar=null, $komentar=null, $idposting=null, $sentimen=null){
		$timezone = new DateTimeZone('Asia/Jakarta');
		$tgl = new DateTime();
		$tgl->setTimeZone($timezone);
		$data = array(
	...
	public function balas_komentar($idpengguna=null, $idkomentar=null, $komentar=null, $sentimen=null){
		$timezone = new DateTimeZone('Asia/Jakarta');
		$tgl = new DateTime();
		$tgl->setTimeZone($timezone);
		$data = array(
	...
	public function statistik_cyberbullying($id){
		$this->db->select('COUNT(komentar.ID_KOMENTAR) AS NILAI, komentar.SENTIMEN AS SENTIMEN');
		$this->db->from('komentar');
		$this->db->join ('posting', 'posting.ID_POSTING = komentar.ID_POSTING' , 'left' );
		$this->db->where('komentar.AKTIF','Y');
		$this->db->where('posting.ID',$id);
		$this->db->group_by('SENTIMEN');
		$this->db->order_by('SENTIMEN','DESC');
		$query = $this->db->get();
        return $query->result();
	}
	...
	public function getArtikelByPopuler(){
		$query = $this->db->query("
			SELECT posting.PHOTO AS PHOTO, posting.TANGGAL AS TANGGAL, pengguna.ID AS ID, pengguna.FIRST_NAME AS FIRST_NAME, pengguna.LAST_NAME AS LAST_NAME, posting.JUDUL_URL, posting.JUDUL, SUM(view.VIEW) AS TOTAL, kategori.NAMA_KATEGORI AS NAMA_KATEGORI FROM posting, view, pengguna, kategori WHERE pengguna.active='1' AND posting.ID_POSTING = view.ID_POSTING AND posting.ID_KATEGORI=kategori.ID_KATEGORI AND posting.ID=pengguna.ID group by posting.ID_POSTING order by TOTAL DESC limit 5");
