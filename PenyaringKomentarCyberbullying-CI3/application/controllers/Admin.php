...
class Admin extends CI_Controller {
	...
	public function artikel_cyberbullying(){
		$pengguna = $this->MyModel->getPenggunaById($_SESSION['user_id']);
		$data['notifikasi'] = $this->MyModel->count_notifikasi($pengguna->ID);
		$data['judul'] = 'Admin | Artikel | Cyberbullying';
		$data['identitas'] = $this->MyModel->getidentitas();			
		$data['kategori'] = $this->MyModel->kategori(); 
		$data['komentar'] = $this->MyModel->get_komentarArtikelByUser($pengguna->ID);
		$this->template('admin/artikel/artikel_cyberbullying.html',$data);
	...
	public function artikel_tinjau($id=null){
		$pengguna = $this->MyModel->getPenggunaById($_SESSION['user_id']);
		$data['notifikasi'] = $this->MyModel->count_notifikasi($pengguna->ID);
		$artikelx = $this->uri->segment(2);
		$komentarx = $this->uri->segment(3);
		if(!is_null($artikelx) && !is_null($komentarx)){
			if(is_numeric($id)){
				...
				if ($this->ion_auth->is_admin()){
					$data['rating'] = $this->MyModel->get_rating($id);
					$data['komentar'] = $this->MyModel->get_komentarArtikel($id);	
					$this->template('admin/artikel/artikel_tinjau.html',$data);	
				}else{
					redirect('error');
				} 
	...
	public function artikel_kelola(){
		...
		if(isset($pengguna)){
			if (!$this->ion_auth->is_admin()){
				...
				if($tinjau->TINJAU_ARTIKEL == "Y"){
				    $data['artikel'] = $this->MyModel->allartikel();
				    $this->template('admin/artikel/artikel_kelola_tinjau.html',$data);
				}else{
				    $data['artikel'] = $this->MyModel->allartikel();
				    $this->template('admin/artikel/artikel_kelola.html',$data);
				}
	...
	public function antibullying_validasi(){
		$data['judul'] = 'Admin | Validasi Komentar';
		$data['identitas'] = $this->MyModel->getidentitas();
		$pengguna = $this->MyModel->getPenggunaById($_SESSION['user_id']);
		$data['notifikasi'] = $this->MyModel->count_notifikasi($pengguna->ID);
		
		$jumlah_data = $this->MyModel->totalvalidasi_datasetkomentar();
		$this->load->library('pagination');
		$config['per_page'] = 9;
		$offset = $this->uri->segment(3);
		$config["uri_segment"] = 3;
		$config["num_links"] = 3;
		$config['base_url'] = base_url().'admin/antibullying_validasi/';
		$config['total_rows'] = $jumlah_data;
		$data['daftar_komentar'] = $this->MyModel->getvalidasi_datasetkomentar($config['per_page'],$offset);
		...
		$this->pagination->initialize($config);
		$this->template('admin/antibullying/antibullying_validasi.html',$data);
	}
