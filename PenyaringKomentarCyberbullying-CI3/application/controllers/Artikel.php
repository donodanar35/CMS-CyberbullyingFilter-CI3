...
class Artikel extends CI_Controller {
    ...
    public function komentar(){
        ...
        if($_POST['idpengguna'] == 0){
            ...
            if($this->form_validation->run()==false){
                ...
                $this->MyModel->komentar($_POST['idpengguna'], $_POST['idposting'],$_POST['komentar'], $_POST['sentimen']);
    ...
    public function kirim_balasan(){
        ...
        if($id==0){
            ...
            if($_POST['komentar']==""){
                ...
                $this->MyModel->balasan_komentar($pengguna->ID, $_POST['idbalasan'], $komentar, $_POST['idposting'], $_POST['sentimen']);
                $callback = array(
                    "status" => 'sukses',
    ...
    public function kirim_balas(){
        ...
        if($id==0){
            ...
            if($_POST['komentar']==""){
                ...
                $this->MyModel->balas_komentar($pengguna->ID, $_POST['idbalas'], $komentar, $_POST['sentimen']);
                $callback = array(
                    "status" => 'sukses',
                    "pesan" => "Komentar balasan berhasil dikirim!"
                );
    ...
    public function kirim_balas_admin(){
        ...
        if($id==0){
            $callback = array(
                "status" => 'gagal',
                "pesan" => "Anda belum login, harap login terlebih dulu!"
            );
        }else{
            $pengguna = $this->MyModel->getPenggunaById($id); 
            if($_POST['komentar']==""){
                ...
                $this->MyModel->balas_komentar($pengguna->ID, $_POST['idkomentar'], $_POST['komentar'], $_POST['sentimen']);
                $callback = array(
                    "status" => 'sukses',
                    "pesan" => "Komentar balasan berhasil dikirim!"
                );
    ...
    public function kirim_balasan_admin(){
        ...
        if($id==0){
            ...
            if($_POST['komentar']==""){
                ...
                $this->MyModel->balas_komentar($pengguna->ID, $_POST['idkomentar'], $komentar, $_POST['sentimen']);
                $callback = array(
                    "status" => 'sukses',
                    "pesan" => "Komentar balasan berhasil dikirim!"
                );
    ...
    public function get_komentarByArtikel($id, $url=""){
        ...
        foreach($komentar->result_array() as $komen){
            ...
            if($id<>0){
                ...
                if(($pengguna->ID) == $komen['ID'] && $komen['SENTIMEN'] =='cyberbullying'){
                    $bully = '<input class="btn btn-outline-dark btn-sm fa fa-exclamation hapus" value="Cyberbullying" type="button" data-toggle="modal" data-id="'. $komen['ID_KOMENTAR'] .'" data-target="#formHapus"/>';
                }
            }
            ...
            $list ='
                <div class="media mb-4">
                    ...
                    <input class="btn btn-outline-warning btn-sm fa fa-exclamation laporkan" value="Laporkan" type="button" data-toggle="modal" data-id="'. $komen['ID_KOMENTAR'] .'" data-target="#formLaporkan"/> '. $hapus . $bully .'
            ...
            foreach($balasan->result_array() as $balas){
                ...
                if($id<>0){
                    ...
                    if(($pengguna->ID) == $balas['ID'] && $balas['SENTIMEN']=='cyberbullying'){
                        $bully_balas = '<input class="btn btn-outline-dark fa fa-exclamation btn-sm hapus-balasan" value="Cyberbullying" type="button" data-toggle="modal" data-id="'. $balas['ID_BALASAN'] .'" data-target="#formHapusBalasan"/>';
                    }
                }
    ...
    public function detail($artikelx = null){
        if(!empty($_SESSION['user_id'])){
			$pengguna = $this->MyModel->getPenggunaById($_SESSION['user_id']);
			$data['notifikasi'] = $this->MyModel->count_notifikasi($pengguna->ID);	
		}else{
		...
		if(isset($artikelx)){
            $data['judul'] = 'Rahmatika.com - Menebarkan kebaikan kepada sesama';
            $artikel = $this->MyModel->getArtikel($artikelx);
            $data['kategori_pilihan'] = $this->MyModel->kategori_pilihan();
    ...
    public function artikel_tinjau(){
		$this->form_validation->set_rules('judul', 'Judul', 'required');
		$this->form_validation->set_rules('deskripsiartikel', 'Deskripsi Artikel', 'required|min_length[5]|max_length[500]');
        $this->form_validation->set_rules('katakunci', 'Kata Kunci', 'required|min_length[5]|max_length[300]');
        $this->form_validation->set_rules('isiartikel', 'Isi Artikel', 'required|min_length[160]');
        ...
        if($this->form_validation->run()==false){
            ...
            $callback = array(
				'status'=>'sukses',
				'pesan'=>'Artikel berhasil diterbitkan!'
            );
            $return = $this->MyModel->posting_tinjau($_POST);
            
                $timezone = new DateTimeZone('Asia/Jakarta');
                $tgl = new DateTime();
                $tgl->setTimeZone($timezone);
                $pengguna = $this->MyModel->getPengguna($_POST['idpengguna']);
                $akunpengikut = $this->MyModel->getpengikut($_POST['idpengguna']);
                //$return = $this->MyModel->artikel_buat($_POST);
                
                if(!empty($akunpengikut) || !is_null($akunpengikut) || $akunpengikut !="" ){
                    ...
                    if(!empty($pengikutku) || !is_null($pengikutku) || $pengikutku !="" ){
	...
	public function artikel_buat(){
        ...
        if($this->form_validation->run()==false){
            ...
            $tinjau = $this->MyModel->getidentitas();
			if($tinjau->TINJAU_ARTIKEL == "N"){
			    $pengguna = $this->MyModel->getPenggunaById($_SESSION['user_id']);
    ...
    public function laporan_rating(){
        $laporan = $this->MyModel->laporan_rating($_POST['idx'], $_POST['tglawal'], $_POST['tglakhir']);
    ...
    public function laporan_artikel(){
        $laporan = $this->MyModel->laporan_artikel($_POST['idx'], $_POST['tglawal'], $_POST['tglakhir']);
        $tulisan = $this->MyModel->get_artikelById($_POST['idx']);
        $tanggal = array();
        $view = array();
        $judul = "";
        ...
        foreach($laporan->result() as $report){
            $tanggal[] = $report->TANGGAL;
            $view[] = $report->VIEW;
            $totalview = $totalview + $report->VIEW;
        }
        $callback = array(
            "sukses" => "sukses",
            "judul" => $tulisan->JUDUL,
    ...
    public function laporan_tayangan(){
        $pengguna = $this->MyModel->getPenggunaById($_POST['idx']);
        $laporan = $this->MyModel->laporan_tayangan($pengguna->ID, $_POST['tglawal'], $_POST['tglakhir']);
        $tanggal = array();
        $view = array();
        $total=0;
        $judul = "Perkembangan Tayangan Seluruh Artikel";
        foreach($laporan->result() as $report){
            $tanggal[] = $report->TANGGAL;
            $view[] = $report->VIEW;
            $total = $total + $report->VIEW;
        }
        $callback = array(
            "sukses" => "sukses",
            "judul" => $judul,
    ...
    public function laporan_top_artikel(){
        $pengguna = $this->MyModel->getPenggunaById($_POST['idx']);
        $laporan = $this->MyModel->getTopArtikel($pengguna->ID, $_POST['tglawal'], $_POST['tglakhir']);
        $judul_artikel = array();
        $view = array();
    ...
    public function laporan_top_rating(){
        $pengguna = $this->MyModel->getPenggunaById($_POST['idx']);
        $laporan = $this->MyModel->getTopRating($pengguna->ID, $_POST['tglawal'], $_POST['tglakhir']);
        $judul_artikel = array();
        $rating = array();
        $rata=0;
        $total =0;
    ...
    public function fpdf(){
        ...
        $pdf->Cell(0,7,'DAFTAR PENGGUNA',0,1,'C');
        $pdf->Cell(10,7,'',0,1);
 
        $pdf->SetFont('Arial','B',10);
        $pdf->Cell(10,6,'No',1,0,'C');
        $pdf->Cell(90,6,'Nama Pegawai',1,0,'C');
        $pdf->Cell(120,6,'Company',1,0,'C');
        $pdf->Cell(40,6,'Telp',1,1,'C');
 
        $pdf->SetFont('Arial','',10);
        $pegawai = $this->db->get('pengguna')->result();
        $no=0;
        foreach ($pegawai as $data){
            $no++;
            $pdf->Cell(10,6,$no,1,0, 'C');
            $pdf->Cell(90,6,$data->FIRST_NAME,1,0);
            $pdf->Cell(120,6,$data->COMPANY,1,0);
            $pdf->Cell(40,6,$data->PHONE,1,1);
        }
        $pdf->Output();
    }

    public function phpoffice(){
        $semua_pengguna = $this->db->get('pengguna')->result();
          ...
          foreach($semua_pengguna as $pengguna) {
               ...
               $spreadsheet->setActiveSheetIndex(0)
                           ...
                           ->setCellValue('C' . $kolom, $pengguna->LAST_NAME)
                           ->setCellValue('D' . $kolom, $pengguna->PHONE)
                           ->setCellValue('E' . $kolom, $pengguna->COMPANY);
    ...
    public function laporan_cyberbullying(){
        //var_dump($this->MyModel->statistik_jawaban());
        $pengguna = $this->MyModel->getPenggunaById($_SESSION['user_id']);
		$laporan = $this->MyModel->statistik_cyberbullying($pengguna->ID);
		$sentimen = array();
		$nilai = array();
        $judul = "Statistik Sentimen Komentar";
        foreach($laporan as $report){
            $sentimen[] = $report->SENTIMEN;
            $nilai[] = $report->NILAI;
        }
        $callback = array(
            "sukses" => "sukses",
