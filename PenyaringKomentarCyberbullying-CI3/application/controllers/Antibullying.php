...
class Antibullying extends CI_Controller {
	...
	public function validasi_komentar(){
        $id = 0;
        ...
        if($id==0){
			...
			$idkomen = $_POST['idkomentar'];
			$jenisbully = "";
			if($_POST['sentimen']=="cyberbullying"){
				$jenisbully = $_POST['jenisbully'];
				$this->MyModel->tambah_riwayatkomentar($pengguna->ID, $_POST['idkomentar'], $_POST['sentimen'],$jenisbully,$_POST['keterangan']);
			}else{
				$jenisbully = "bukan perundungan";
				$this->MyModel->tambah_riwayatkomentar($pengguna->ID, $_POST['idkomentar'], $_POST['sentimen'], $jenisbully,$_POST['keterangan']);
			}
			$this->MyModel->validasi_komentar($_POST['idkomentar'], $_POST['sentimen'], $jenisbully, $_POST['keterangan']);
			$callback = array(
                "status" => 'sukses',
                "pesan" => "Komentar #$idkomen berhasil divalidasi!"
        	);
        }
        echo json_encode($callback);
	}

	public function simpan_validasi_komentar(){
		$idkomen = $_POST['idkomentar'];
		$jenisbully = "";
		if($_POST['sentimen']=='cyberbullying'){
			$jenisbully = $_POST['jenisbully'];
		...
		$this->MyModel->simpan_validasi_komentar($_POST['idkomentar'], $_POST['isikomentar'], $_POST['sentimen'], $jenisbully);
		$callback = array(
                "status" => 'sukses',
                "pesan" => "Komentar #$idkomen berhasil disimpan!"
		);
		echo json_encode($callback);
	}
	
	Public function get_riwayatvalidasi(){
		$riwayatkomentar = $this->MyModel->get_validasibyid($_POST['idkomentar']);
		$mytabel = '<table class="table table-hover"><tr><td><center><b>ID</b></center></td><td><center><b>Waktu</b></center></td><td><center><b>Validator</b></center></td><td><center><b>Sentimen</b></center></td><td><center><b>Jenis Sentimen</b></center></td><td><center><b>Keterangan</b></center></td></tr>';
		$callback = array();
		//var_dump($riwayatkomentar);
		foreach($riwayatkomentar as $riwayat){
			//$mytabel = $mytabel . $riwayat->WAKTU;
			$mytabel = $mytabel . "<tr><td>" . $riwayat->ID_VALIDASI . "</td><td>" . $riwayat->WAKTU . "</td><td>" . $riwayat->FIRST_NAME . "</td><td>" . $riwayat->SENTIMEN . "</td><td>" . $riwayat->JENIS_CYBERBULLYING . "</td><td>" . $riwayat->KETERANGAN . "</td></tr>";
		}
		$mytabel = $mytabel . "</table>";
		$callback = array(
			"status" => "sukses",
			"tabel" => $mytabel
		);
	...
	public function get_datasetkomentar()
	{
		...
		foreach($list->result() as $li){
			...
			$row[] = $li->STOPWORD;
			$row[] = $li->TOKENIZING;
			if($li->SENTIMEN=='non-cyberbullying'){
				$row[] = '<center>
				<a href="#" class="btn btn-primary konfir-ubah" data-toggle="modal" data-target="#formUbah" data-idb="'.  $li->ID_DATASETKOMENTAR .'" data-idc="'. $li->SENTIMEN .'"  data-idd="'. $li->JENIS_CYBERBULLYING.'" data-idi="'. $li->ISI_KOMENTAR .'">N</a></center>';
			}else{
				$row[] = '<center>
				<a href="#" class="btn btn-danger konfir-ubah" data-toggle="modal" data-target="#formUbah" data-idb="'.  $li->ID_DATASETKOMENTAR .'" data-idc="'. $li->SENTIMEN .'"  data-idd="'. $li->JENIS_CYBERBULLYING.'" data-idi="'. $li->ISI_KOMENTAR .'">Y</a></center>';
			}
			$data[] = $row;
		}	
		$output = array(
